...
const OAUTH_PROVIDERS = [
  ...
  },
  {
    ...
    label: "LinkedIn",
    icon: (
      <span className="flex h-5 w-5 items-center justify-center rounded bg-[#0A66C2] text-[13px] font-bold leading-none text-white" aria-hidden="true">
        in
      </span>
    ),
  },
    ...
    <Card className="shadow-glass border-border/50">
      ...
      <CardContent>
        ...
        <p className="mt-4 text-center text-sm text-mossy-gray">
          ...
          <Link href={from && from !== "/dashboard" ? `/login?from=${encodeURIComponent(from)}` : "/login"} className="text-state-today hover:underline font-medium">
            Sign in
          </Link>
        </p>
      </CardContent>
