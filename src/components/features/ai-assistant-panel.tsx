...
export function AiAssistantPanel({ kind, contextId, title, enableSummary = true, onClose }: AiAssistantPanelProps) {
  ...
  React.useEffect(() => {
    if (enableSummary && tab === "summary" && summary === null && !summaryLoading && !summaryError) {
      void loadSummary(false);
    }
  }, [tab, enableSummary, summary, summaryLoading, summaryError, loadSummary]);
  ...
  async function send() {
    ...
    try {
      ...
      setMessages((m) => {
        ...
        copy[copy.length - 1] = {
          role: "assistant",
          content: "Warning: " + (e instanceof Error ? e.message : "Something went wrong."),
        };
        return copy;
      });
    } finally {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[60] flex h-[min(70vh,560px)] w-[min(92vw,400px)] flex-col rounded-2xl border border-border bg-surface shadow-hover print:hidden">
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        ...
        <button onClick={onClose} className="p-1 text-mossy-gray hover:text-forest-slate" aria-label="Close">
          <X className="h-4 w-4" />
        </button>
      </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-3">
            {summaryLoading && (
              <div className="flex items-center gap-2 text-sm text-mossy-gray">
                <Loader2 className="h-4 w-4 animate-spin" /> Summarising <span>{title}</span>...
              </div>
            )}
          ...
          <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto px-4 py-3">
            {messages.length === 0 && (
              <p className="text-sm text-mossy-gray">
                Ask anything about <strong>{title}</strong> - definitions, examples, <span>explain this part</span>, <span>quiz me</span>.
              </p>
            )}
            {messages.map((m, i) => (
